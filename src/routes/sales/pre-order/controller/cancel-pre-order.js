...
const cancel_pre_order = async (request, res) => {
    ...
    try {
        ...
        if (current.status !== "Pending" && current.status !== "Confirmed") {
            await client.query("ROLLBACK");
            return res.status(409).json({ code: 409, message: `This pre-order cannot be cancelled (status ${current.status})` });
        }

        if (advance_refunded > Number(current.advance_paid)) {
            await client.query("ROLLBACK");
            return res.status(400).json({ code: 400, message: `Refund cannot exceed the advance paid (${current.advance_paid})` });
        }
        ...
        try {
            ...
            log.error(`Rollback failed on cancel pre-order: ${rollbackError?.message}`);
        }
