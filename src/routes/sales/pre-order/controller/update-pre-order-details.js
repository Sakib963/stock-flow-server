...
const update_pre_order_details = async (request, res) => {
    ...
    try {
        ...
        if (current.status === "Converted" || current.status === "Cancelled") {
            await client.query("ROLLBACK");
            return res.status(409).json({ code: 409, message: `A ${current.status} pre-order can no longer be edited` });
        }
        ...
        if (linesEditable) {
            ...
            subtotal = Number(sub.rows[0].subtotal);
        }
        ...
        if (linesEditable) {
            await client.query({ text: `DELETE FROM ${TABLE.PRE_ORDER_ITEMS} WHERE pre_order_oid = $1`, values: [payload.oid] });
        ...
        try {
            ...
            log.error(`Rollback failed during pre-order update: ${rollbackError?.message}`);
        }
