...
const { product_list_schema, product_schema } = require("./schema");
...
// Get Product Details
router.get(
  ROUTES.GET_PRODUCT_DETAILS + "/:oid",
  [jwtMiddleware],
  get_product_details,
);

// Delete Product
router.get(ROUTES.DELETE_PRODUCT + "/:oid", [jwtMiddleware], delete_product);

// Generate Product Inventory Report
router.post(
  ROUTES.GENERATE_PRODUCT_INVENTORY_REPORT,
  [jwtMiddleware],
  generate_product_inventory_report,
);

// Generate Product Movement Report
router.post(
  ROUTES.GENERATE_PRODUCT_MOVEMENT_REPORT,
  [jwtMiddleware],
  generate_product_movement_report,
);
