...
const update_supplier_details = async (request, res) => {
      ...
      try {
            ...
            const oldDataSql = {
                  text: `SELECT name, contact_person, phone_number, email, address, status FROM ${TABLE.SUPPLIER} WHERE oid = $1`,
                  values: [payload.oid]
            };
            const oldData = await get_data(oldDataSql);
            const oldSupplier = oldData.length ? oldData[0] : null;
            ...
            if (oldSupplier) {
                  ...
                  const changes = detectChanges(oldSupplier, payload);
                  const activityDescription = generateChangeDescription(`supplier "${payload.name}"`, changes);
