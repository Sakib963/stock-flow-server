...
const create_brand = async (request, res) => {
      ...
      try {
            ...
            if (exiting_brand) {
                  log.warn(`Brand already exists [${payload.name}]`);
                  return res.status(409).json({ code: 409, message: "Brand Already Exists!" });
            }

            const brandOid = uuidv4();
            const sql = {
                  text: `INSERT INTO ${TABLE.BRANDS} (oid, name, description, status, created_by) VALUES ($1, $2, $3, $4, $5)`,
                  values: [brandOid, payload.name, payload.description, payload.status, user_id]
            }
