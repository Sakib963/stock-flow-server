...
const generate_count_sql = (request) => {
      let query = `SELECT COUNT(*) AS total FROM ${TABLE.BRANDS} b WHERE 1 = 1`;
      let values = [];

      if (request.query.search_text && request.query.search_text.trim() !== "") {
            const searchText = `%${request.query.search_text.trim().toLowerCase()}%`;
            query += ` AND (LOWER(b.name) LIKE $${values.length + 1} `;
            query += `OR LOWER(b.description) LIKE $${values.length + 2})`;
...
const generate_data_sql = (request) => {
      let query = `SELECT b.oid, b.name, b.description, b.status, b.created_by, b.created_on FROM ${TABLE.BRANDS} b  WHERE 1 = 1`;
      let values = [];
