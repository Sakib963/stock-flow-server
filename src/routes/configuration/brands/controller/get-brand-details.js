...
const get_brand_details = async (request, res) => {
      try {
            ...
            const detailsSql = generate_details_sql(brandOid);
            ...
            // Step 2: Get brand statistics
            const statsSql = generate_stats_sql(brandOid);
            ...
            const activity_set = await getLogActivities('brand', brandOid, 10);
            ...
            log.info(`Brand details found for oid: ${brandOid}`);
...
const generate_details_sql = (brandOid) => {
      const query = `
            SELECT 
                  b.oid, 
                  b.name, 
...
const generate_stats_sql = (brandOid) => {
      const query = `
