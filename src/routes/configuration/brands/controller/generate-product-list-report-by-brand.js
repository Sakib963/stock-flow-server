...
const generate_product_list_report_by_brand = async (request, res) => {
      try {
            ...
            if (!products || products.length === 0) {
                  log.info(`No products found for brand OID: ${brandOid}`);
            ...
            // Extract brand details from first row (same for all products)
            const brandDetails = {
                  name: products[0].brand_name,
                  description: products[0].brand_description,
                  status: products[0].brand_status,
            };
            
            const buffer = await generate_products_xlsx(products, brandDetails);
            const timestamp = Date.now();
            const file_name = `${brandDetails.name.replace(/\s+/g, '_')}_products_${timestamp}.xlsx`;

            log.info(`Download products list for brand [${brandDetails.name}] - [${file_name}]`);
...
const generate_products_sql = (brandOid) => {
      const query = `
            SELECT 
                  p.oid,
            ...
            WHERE p.brand_oid = $1
            GROUP BY p.oid, p.name, p.sku, p.status, p.restock_threshold, p.unit_type, s.name, c.name, b.name, b.description, b.status, ps.total_sold, ps.total_returned
            ORDER BY p.name ASC
      `;
      return { text: query, values: [brandOid] };
};

const generate_products_xlsx = async (products, brandDetails) => {
      ...
      addReportHeader(sheet, `Products List - ${brandDetails.name}`, titles.length);
...
module.exports = generate_product_list_report_by_brand;
