...
const generate_inventory_report_by_warehouse = async (request, res) => {
  try {
    ...
    // Get inventory with warehouse details in a single query
    const inventorySql = generate_inventory_sql(warehouseOid);
    const inventory = await get_data(inventorySql);

    if (!inventory || inventory.length === 0) {
      log.info(`No inventory found for warehouse OID: ${warehouseOid}`);
    ...
    // Extract warehouse details from first row (same for all inventory items)
    const warehouseDetails = {
      name: inventory[0].warehouse_name_detail,
      code: inventory[0].warehouse_code,
      location: inventory[0].warehouse_location,
      status: inventory[0].warehouse_status,
    };

    const buffer = await generate_inventory_xlsx(inventory, warehouseDetails);
    const timestamp = Date.now();
    const file_name = `${warehouseDetails.name.replace(/\s+/g, "_")}_inventory_report_${timestamp}.xlsx`;
    ...
    log.info(
      `Download inventory report for warehouse [${warehouseDetails.name}] - [${file_name}]`,
    );
    res
      .set(
        "Content-Type",
    ...
    log.error(
      `An exception occurred while generating inventory report by warehouse: ${e?.message}`,
    );
    console.error(e);
...
const generate_inventory_sql = (warehouseOid) => {
  const query = `
            SELECT
                  ...
                  w.name AS warehouse_name_detail,
...
const generate_inventory_xlsx = async (inventory, warehouseDetails) => {
  ...
  addReportHeader(
    sheet,
    `Inventory Report - ${warehouseDetails.name}`,
    titles.length,
  );
...
module.exports = generate_inventory_report_by_warehouse;
