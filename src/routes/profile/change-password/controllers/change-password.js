...
const change_password = async (request, res) => {
  ...
  try {
    const { isValid, user_name } = await check_current_password(
    ...
    let { otp_oid, otp } = await save_generated_otp(
      request.credentials.user_id,
    );
    if (!otp_oid) {
      return res
        .status(500)
        .json({ code: 500, message: "Failed to generate OTP." });
    }
    ...
    log.error(
      `An exception occurred while getting user information: ${e?.message}`,
    );
    return res
      .status(500)
      .json({
...
const check_current_password = async (user_id, current_password) => {
  ...
  try {
    ...
    log.error(
      `An exception occurred while getting user password information: ${e?.message}`,
    );
  }
...
const send_otp = async (request, otp, user_name) => {
  ...
  html = html.replace(/{{HELP_CENTER_URL}}/g, COMPANY_INFO.helpCenterUrl);
  html = html.replace(/{{LOGO_URL}}/g, COMPANY_INFO.publicLogoUrl);

  const options = {
    to: request.credentials.user_id,
    subject: `OTP Verification - ${COMPANY_INFO.appName}`,
    html,
  };
