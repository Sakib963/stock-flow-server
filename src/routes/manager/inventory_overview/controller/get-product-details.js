...
const get_product_details = async (request, res) => {
  try {
    ...
    try {
      ...
      log.error(
        `An exception occurred while getting Product details: ${e?.message}`,
      );
      throw e;
    }
    try {
      ...
      log.error(
        `An exception occurred while getting Product batch list: ${e?.message}`,
      );
      throw e;
    }
    ...
    log.error(
      `An exception occurred while getting Purchase details: ${e?.message}`,
    );
    return res
      .status(500)
      .json({
