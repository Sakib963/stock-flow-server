...
const get_current_stock_report = async (request, res) => {
  try {
    ...
    res
      ...
      .set("X-Filename", file_name_encoded) // <-- new header
...
const get_current_stock_data = async (request) => {
  ...
  try {
    ...
    // FILTER: CATEGORY
    if (
      payload.category_oid &&
      payload.category_oid.trim() !== "" &&
      payload.category_oid.trim().toLowerCase() !== "null"
    ) {
    ...
    // FILTER: SUB CATEGORY
    if (
      payload.sub_category_oid &&
      payload.sub_category_oid.trim() !== "" &&
      payload.sub_category_oid.trim().toLowerCase() !== "null"
    ) {
    ...
    log.error(
      `An exception occurred while getting current stock report information: ${err?.message}`,
    );
    throw err;
  }
  return data;
